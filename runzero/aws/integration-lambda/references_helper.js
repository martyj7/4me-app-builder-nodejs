...
class ReferencesHelper {
  ...
  async getSoftwareReferences(accessToken, assets) {
    const softwareNames = this.runzeroHelper.extractSoftwareNames(assets);
    const osNames = this.runzeroHelper.extractOperatingSystemNames(assets);
  ...
  async lookupAllSoftware(accessToken) {
    ...
    if (!software.error) {
      software.forEach(s => {
        s.name = this.runzeroHelper.cleanupName(s.name);
        if (s.alternateNames) {
          s.alternateNames = s.alternateNames.map(n => this.runzeroHelper.cleanupName(n));
        }
      });
